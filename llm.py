...
def summarize_with_openai(text, api_key=None, model="gpt-3.5-turbo"):
    ...
    try:
        ...
        # Construct the prompt for the user message
        user_prompt = f"Создай короткое описание (2-3 предложения) для видео на основе этого текста:\n\n{text[:4000]}"

        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "Ты — помощник, который анализирует тексты видео. "
                               "Создай краткое, привлекательное описание на основе транскрипта. "
...
def summarize_with_ollama(text, model="llama3", ollama_url="http://localhost:11434"):
    ...
    """
    try:
        prompt = f"""Ты — помощник для создания описаний.
Создай краткое, привлекательное описание (2-3 предложения) для видео на основе этого текста:

{text[:3000]}

Описание должно быть интересным и завлекать зрителя.
